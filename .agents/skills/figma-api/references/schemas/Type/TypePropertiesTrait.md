...
|-------|------|----------|-------------|
| `characters` | string | Yes | The raw characters in the text node. |
| `style` | [TypeStyle](TypeStyle.md) | Yes |  |
...
| `styleOverrideTable` | object | Yes | Map from ID to TypeStyle for looking up style overrides. |
| `lineTypes` | string[] | Yes | An array with the same number of elements as lines in the text node, where lines are delimited by newline or paragraph separator characters. Each element in the array corresponds to the list type of a specific line. List types are represented as string enums with one of these possible values:

- `NONE`: Not a list item.
- `ORDERED`: Text is an ordered list (numbered).
- `UNORDERED`: Text is an unordered list (bulleted). |
| `lineIndentations` | number[] | Yes | An array with the same number of elements as lines in the text node, where lines are delimited by newline or paragraph separator characters. Each element in the array corresponds to the indentation level of a specific line. |
